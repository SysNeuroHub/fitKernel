...
if ispc
    ...
    saveFigFolder = 'E:/tmp/cuesaccade_data/20220716/';
    mkdir(saveFigFolder);
elseif isunix
...
thisdata = find(1-cellfun(@isempty, regexp(loadNames, ...
    regexptranslate('wildcard','12December\07\*_ch1.mat'))));
...
previousDate = [];
for idata = 800%:length(channels) %1061;%865;%
    ...
    if allTr
        saveFigName = fullfile(saveFigFolder, ['cellclassFig_' saveSuffix '_allTr.fig']);
    else
    ...
    if exist(loadName,'file')>0
       ...
       load(loadNames{idata},'dd');
        ...
        load(fullfile(saveFolder,['eyeCat_' animal thisDate '.mat']),'catEvTimes',...
       ...
       %validEvents = intersect(find(~isnan(onset)), find(dd.cueOn==icue-1));
        validEvents = intersect(find(~isnan(onset)), find(dd.successTrials));
        ...
        if isempty(fieldnames(test))
            ...
            load(fullfile(saveFolder,['eyeCat_' animal thisDate '.mat']), 'startSaccNoTask', 'endSaccNoTask', ...
       ...
       [avgOnsetResp_v, winSamps, singleOnsetResp_v, ...
           ...
           = eventLockedAvg(cat(1,PSTH_f', predicted_all', predicted'), ...
           predictorInfo.t_r, onsetTimes, tgtDir_v, param.figTWin);
