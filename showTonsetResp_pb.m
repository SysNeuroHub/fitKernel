function [f, cellclassInfo] = showTonsetResp_pb(t_r, y_r, catEvTimes, dd, psthNames, ...
    startSaccNoTask, saccDirNoTask, param, allTr, includeTrials, withOriginal)
...
pupilIdx = find(strcmp(psthNames, 'pdiam'));
blinkIdx = find(strcmp(psthNames, 'blink'));
...
mtOnsetRespAmp = mean(tonsetRespAmp(theseTrials,[psthIdx pupilIdx blinkIdx]),1);
%mtOnsetRespAmp = mtOnsetRespAmp_p_c - mtOnsetRespAmp_b_c;

if ~isempty(theseTrials)
    [PtonsetResp_paired] = signrank(tonsetRespAmp(theseTrials,pupilIdx), ...
        tonsetRespAmp(theseTrials,blinkIdx));
else 
...
hold on;
boundedline(winSamps, mtOnsetResp(pupilIdx,:), setOnsetResp(pupilIdx,:),'m', 'transparency', 0.5);
if ~isempty(blinkIdx)
    boundedline(winSamps, mtOnsetResp(blinkIdx,:), setOnsetResp(blinkIdx,:),'c', 'transparency', 0.5);
end
set(gca,'tickdir','out');
...
hold on;
boundedline(winSamps, mtOnsetResp_v(pupilIdx,:), setOnsetResp_v(pupilIdx,:),'m', 'transparency', 0.5);
if ~isempty(blinkIdx)
    boundedline(winSamps, mtOnsetResp_v(blinkIdx,:), setOnsetResp_v(blinkIdx,:),'c', 'transparency', 0.5);
end
...
% end
% boundedline(winSamps, mtOnsetResp_f(eyeposIdx,:), setOnsetResp_f(eyeposIdx,:),'c', 'transparency', 0.5);
% %vbox(param.baseWin(1), param.baseWin(2))
% %vbox(param.tOnRespWin(1), param.tOnRespWin(2),[],[.7 1 .7]);
% axis tight;
...
hold on;
boundedline(winSamps+medianSaccDelay, msaccResp(pupilIdx,:), sesaccResp(pupilIdx,:),'m', 'transparency', 0.5);
if ~isempty(blinkIdx)
    boundedline(winSamps+medianSaccDelay, msaccResp(blinkIdx,:), sesaccResp(blinkIdx,:),'c', 'transparency', 0.5);
end
axis tight;
...
% end
% boundedline(winSamps, mcOnsetResp(eyeposIdx,:), secOnsetResp(eyeposIdx,:),'c', 'transparency', 0.5);
% axis tight;
% xlim([figTWin(1)-medianSaccDelay figTWin(end)-medianSaccDelay])

linkaxes(ax(:)); 
vline(0, ax(1), '--','m'); vline(medianSaccDelay, ax(1), '--','c');
vline(0, ax(2),'--','m');
vline(medianSaccDelay, ax(3),'--','c');
...
if withOriginal
legend('Empirical','Pupil diameter','Blink','location','northwest')
else
legend('Pupil diameter','Blink','location','northwest')
end    
end
